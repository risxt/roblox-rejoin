...
Usage:
  python reconnect.py --url "PRIVATE_SERVER_URL" --packages "pkg1,pkg2"
  python reconnect.py --config  (uses saved config)
...
╔═══════════════════════════════════════════════════════════╗
║     🎮 Roblox Auto-Reconnect Tool v1.0                    ║
║     For Termux (Cloud Phone)                              ║
╠═══════════════════════════════════════════════════════════╣
║  • Multi-instance monitoring                              ║
║  • Auto-reconnect to Private Server                       ║
║  • Root-based process detection                           ║
╚═══════════════════════════════════════════════════════════╝
{Colors.RESET}"""
        print(banner)
    ...
    def launch_roblox(self, package: str):
        ...
        if success:
            self.log(f"✅ Launched {package} successfully", "SUCCESS")
        else:
            self.log(f"❌ Failed to launch {package}: {output}", "ERROR")
    ...
    def interactive_setup(self):
        ...
        print(f"\n{Colors.BOLD}📋 Configuration Setup{Colors.RESET}\n")
        
        # Check for existing config
        if self.load_config():
            print(f"{Colors.GREEN}Found existing configuration:{Colors.RESET}")
            print(f"  • Place ID: {self.config.place_id}")
            print(f"  • Link Code: {self.config.link_code[:10]}...")
            print(f"  • Packages: {', '.join(self.config.packages)}")
            print()
        ...
        while True:
            ...
            if place_id and link_code:
                ...
                print(f"  {Colors.RED}❌ Could not parse URL. Please try again.{Colors.RESET}")
        ...
        while True:
            ...
            if packages:
                print(f"\n  {Colors.GREEN}✅ {len(packages)} package(s) configured:{Colors.RESET}")
                for pkg in packages:
                    print(f"     • {pkg}")
                break
            else:
                print(f"  {Colors.RED}❌ Please enter at least one package name.{Colors.RESET}")
    ...
    def monitor_loop(self):
        ...
        print(f"\n{Colors.GREEN}{Colors.BOLD}🚀 Starting monitor...{Colors.RESET}")
        ...
        while self.running:
            for package in self.config.packages:
                ...
                if is_running:
                    self.log(f"✓ {package} is running", "INFO")
                else:
                    self.stats[package]["crashes"] += 1
                    self.stats[package]["last_crash"] = datetime.now().strftime("%H:%M:%S")
                    
                    self.log(f"💥 CRASH DETECTED: {package} (Total: {self.stats[package]['crashes']})", "CRASH")
                    
                    # Launch the app
                    if self.launch_roblox(package):
                        self.log(f"⏳ Waiting {self.config.launch_delay}s for app to load...", "INFO")
    ...
    def graceful_shutdown(self, signum=None, frame=None):
        ...
        for pkg, stats in self.stats.items():
            print(f"   • {pkg}: {stats['crashes']} crash(es)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Roblox Auto-Reconnect Tool for Cloud Phone',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  python reconnect.py --url "https://roblox.com/games/123?privateServerLinkCode=ABC" --packages "com.roblox.client1,com.roblox.client2"
  python reconnect.py --config
  python reconnect.py --url "roblox://..." --packages "com.roblox.client1" --debug
        '''
    )
    parser.add_argument('--url', '-u', help='Private Server URL')
    parser.add_argument('--packages', '-p', help='Package names (comma separated)')
    parser.add_argument('--interval', '-i', type=int, default=5, help='Check interval in seconds (default: 5)')
    parser.add_argument('--delay', '-d', type=int, default=30, help='Launch delay in seconds (default: 30)')
    parser.add_argument('--debug', action='store_true', help='Enable debug mode')
    parser.add_argument('--config', '-c', action='store_true', help='Use saved config file')
    
    args = parser.parse_args()
    
    tool = RobloxReconnect()
    
    # If command line args provided, skip interactive setup
    if args.url and args.packages:
        tool.print_banner()
        
        # Check root
        if not tool.check_root_access():
            print(f"\n{Colors.RED}Please enable root access and try again.{Colors.RESET}")
            sys.exit(1)
        
        tool.optimize_android()
        
        # Parse URL
        place_id, link_code = tool.parse_private_server_url(args.url)
        if not place_id or not link_code:
            print(f"{Colors.RED}Error: Could not parse URL{Colors.RESET}")
            sys.exit(1)
        ...
        tool.config = Config(
            ...
            launch_delay=args.delay,
            debug_mode=args.debug
        )
        tool.debug_mode = args.debug
        tool.stats = {pkg: {"crashes": 0, "last_crash": None} for pkg in packages}
        
        print(f"{Colors.GREEN}Config from arguments:{Colors.RESET}")
        print(f"  Place ID: {place_id}")
        ...
        if tool.load_config():
            tool.debug_mode = tool.config.debug_mode
            tool.stats = {pkg: {"crashes": 0, "last_crash": None} for pkg in tool.config.packages}
            print(f"{Colors.GREEN}Loaded saved config{Colors.RESET}")
            try:
                tool.monitor_loop()
            except KeyboardInterrupt:
                tool.graceful_shutdown()
        else:
            print(f"{Colors.RED}No saved config found. Use --url and --packages{Colors.RESET}")
            sys.exit(1)
    else:
